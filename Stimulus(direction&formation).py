...
qr_forward = QRCode(posX = w//2 ,
             posY = h//2 - marginY,
             frequency = 7,
             size = direction_size)

qr_left = QRCode(posX = w//2 - marginX,
             posY = h//2,
             frequency = 10,
             size = direction_size)

qr_right = QRCode(posX = w//2 + marginX,
             posY = h//2 ,
             frequency = 13,
             size = direction_size)

qr_backward = QRCode(posX = w//2,
             posY = h//2 + marginY,
             frequency = 17,
             size = direction_size)

qr_formation_1 = QRCode(posX = formation_size,
             posY = formation_size,
             frequency = 23,
             size = formation_size)

qr_formation_2 = QRCode(posX = w - formation_size,
             posY = formation_size,
             frequency = 29,
             size = formation_size)

qr_formation_3 = QRCode(posX = formation_size,
             posY = h - formation_size,
             frequency = 31,
             size = formation_size)

qr_formation_4 = QRCode(posX = w - formation_size,
             posY = h - formation_size,
             frequency = 37,
             size = formation_size)
...
while(1):
    if (time.time() <= timestamp):
        ...
        if (time.time() <= timestamp2):
            if(s<4):
                qr_forward.flick(img)
                qr_left.flick(img)
                qr_right.flick(img)
                qr_backward.flick(img)
                qr_formation_1.flick(img)
                qr_formation_2.flick(img)
                qr_formation_3.flick(img)
                qr_formation_4.flick(img)
            if s == 0:
                color = (0, 0, 255)
                thickness = 5

                start_point =(w//2 - direction_size//2 - 10, h//2 - marginY - direction_size//2 - 10)
                end_point =(w//2  + direction_size//2 + 10, h//2 - marginY + direction_size//2 + 10)
                ...
                start_point =(w//2 - marginX - direction_size//2 - 10, h//2  - direction_size//2 - 10)
                end_point =(w//2 - marginX + direction_size//2 + 10, h//2  + direction_size//2 + 10)
